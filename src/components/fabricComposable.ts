...
export function useFabricCanvas() {
  ...
  const setBackground = (width: number, height: number) => {
  ...
  const init = (
    canvasElementRef: Ref<HTMLCanvasElement | undefined>,
    _canvasWidth: number,
    ...
    if (canvasElementRef.value) {
      ...
      setBackground(canvasWidth, canvasHeight);
      ...
      canvas.on("mouse:down", function (options) {
        ...
        if (
          currentTime - lastClickedTime < doubleClickThreshold &&
          options.target
        ) {
